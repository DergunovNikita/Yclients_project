...
- **Ветка**: `feature/demo-mode`, коммиты поблочно.
...
6. `PortalUser(email='demo@portal.local', role='owner', is_demo=True, portal_account_id, is_active=True, email_verified_at=now)`, `password_hash=hash_password(generate_initial_password())`.
...
## Блок F — Тесты
**Файлы:** `tests/test_auth_demo.py` (+ дополнить существующие)
