...
from models import Company, PortalAccount, PortalBranch, PortalUser, Staff
...
from yclients_credential_routes import router as credential_router
from config import (
...
router = APIRouter()
# YClients credential admin (create/list/update/delete/test) lives in its own router; mounted
# here so it rides along under both `/auth` and `/dashboard/auth` (see api.py) with no change
# there.
...
class RegisterRequest(BaseModel):
    email: EmailStr
    password: str = Field(min_length=PORTAL_PASSWORD_MIN_LENGTH, max_length=128)
...
class ChangePasswordRequest(BaseModel):
    ...
    new_password: str = Field(min_length=PORTAL_PASSWORD_MIN_LENGTH, max_length=128)
...
class AdminStaffCreateAccountRequest(BaseModel):
    ...
    role: str | None = None
    password: str | None = Field(default=None, min_length=PORTAL_PASSWORD_MIN_LENGTH, max_length=128)
...
async def _tenant_scoped_users(
    db: AsyncSession,
    portal_account_id: int | None,
    *conditions,
) -> list[PortalUser]:
    """List portal users within one resolved tenant, for the admin user-listing endpoints.

    `None` means no tenant was resolved (an owner who registered but has not onboarded yet) and
    must return nothing: `PortalUser.portal_account_id == None` compiles to SQL `IS NULL`, which
    would otherwise match every other not-yet-onboarded owner across every tenant.
    """
    if portal_account_id is None:
        return []
    stmt = (
        select(PortalUser)
        .where(PortalUser.portal_account_id == portal_account_id, *conditions)
        .order_by(PortalUser.id.asc())
    )
...
async def admin_list_users(
    ...
    users = await _tenant_scoped_users(db, active_portal_account_id)
    if active_portal_account_id is not None:
        # Scoped to the resolved tenant only: unscoped, this repaired every tenant's Staff
        # mirror rows on any admin's GET — a branch_admin of tenant A writing and committing
        # rows for tenant B. See portal_staff_sync.sync_all_portal_users_staff.
        await sync_all_portal_users_staff(db, active_portal_account_id)
    await db.commit()
...
async def admin_list_initial_passwords(
    ...
    active_portal_account_id = await _active_admin_portal_account_id(db, actor, x_portal_account_id)
    actor_branch_ids = await _active_admin_branch_ids(db, actor, x_portal_account_id)
    users = await _tenant_scoped_users(
        db,
        active_portal_account_id,
        PortalUser.is_active.is_(True),
        PortalUser.is_demo.is_(False),
        PortalUser.last_login_at.is_(None),
    )
    staff_ids_by_user = await _load_staff_ids_by_portal_user(db, [user.id for user in users])
