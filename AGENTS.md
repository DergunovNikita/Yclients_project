...
- База для дельты (`_previous_period_countable_months`) — столько же месяцев непосредственно
  ...
  месяцем. Считать её по окну `previous_period()` нельзя: оно отступает днями, для февраля
  начиналось бы 2-го числа и не засчитало бы ничего, а квартал-к-дате получил бы на месяц
  меньше — дельта показывала бы рост на пустом месте.
