...
- В обзоре `visit_metrics.opz_qty` показывает количество ОПЗ, а `opz_pct` — долю от завершённых визитов. Дневные поля `opz_qty` и `opz_pct` доступны в `revenue_daily`.
